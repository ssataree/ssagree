...
public class Main : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //poolManager.Initialize(_listOfGems, 16);
        ...
        for (int i = 0; i <= _gridWidth - 1; i++)
        {
            ...
            {
                //var go = poolManager.Get(Random.Range(0, _listOfGems.Length), new Vector3(i, j, 0), transform.rotation);
    ...
    {//creating an arraylist to store the matching tiles
        ...
        for (var x = 0; x <= cells.GetUpperBound(0); x++)
        {
            for (var y = 0; y <= cells.GetUpperBound(1); y++)
            {
                ...
                if (null == thiscell.GetComponent<PoolObject>()) continue;
                int matchCount = 0;
                ...
                for (y1 = y + 1; y1 <= y2; y1++)
                {
                    ...
                    if (cells[x, y1].GetComponent<PoolObject>() == null || thiscell.name != cells[x, y1].name) break;
                    matchCount++;
                }
        ...
        for (int x = 0; x <= cells.GetUpperBound(0); x++)
        {
            for (int y = 0; y <= cells.GetUpperBound(1); y++)
            {
                if (cells[x, y].name == "Empty(Clone)")
                {
                    ...
                    //cells[x, y] = poolManager.Get(Random.Range(0, _listOfGems.Length), new Vector3(x, cells.GetUpperBound(1) + 2, 0), transform.rotation);
